...
export default function MealPlanner() {
  ...
  const [wasteCost, setWasteCost] = useState("");  // ✅ FIX: proper React state instead of getElementById
  ...
  // ✅ FIX: log waste using React state values, no DOM access
  const logWaste = () => {
    if (!wasteInput.trim()) return;
    setWasteLog((prev) => [
      ...prev,
      { name: wasteInput.trim(), cost: parseFloat(wasteCost) || 0, week: activeWeek, id: Date.now() },
    ]);
    setWasteInput("");
    setWasteCost("");
            ...
            {addingWaste ? (
              <div style={{ background: "#FFF", borderRadius: 14, padding: 18, border: "1px solid #E5E0D4", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  {/* ✅ FIX: both inputs now use React state — no getElementById */}
                  <input
                    value={wasteInput}
                    onChange={(e) => setWasteInput(e.target.value)}
                    placeholder="Item name (e.g. spinach, leftover soup)"
                    style={{ flex: 2, minWidth: 160, padding: "9px 13px", borderRadius: 8, border: "1.5px solid #D8D2C4", fontSize: 13, fontFamily: "inherit" }}
                  />
                  <input
                    type="number"
                    value={wasteCost}
                    ...
                    placeholder="Est. $"
                    style={{ flex: 1, minWidth: 80, padding: "9px 13px", borderRadius: 8, border: "1.5px solid #D8D2C4", fontSize: 13, fontFamily: "inherit" }}
                  />
                  <button onClick={logWaste} style={{ padding: "9px 18px", background: "#e53935", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontWeight: 600, fontSize: 13 }}>Log</button>
